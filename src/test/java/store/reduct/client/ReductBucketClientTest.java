...
class ReductBucketClientTest {
	...
	// bucketClient = new ReductBucketClient(serverProperties, httpClient,
	// ACCESS_TOKEN);
	// mockHttpResponse = mock(HttpResponse.class);
	// }
	//
	// @Test
	// void createBucket_settingsNotProvided_bucketCreated() throws IOException,
	// InterruptedException {
	// String createBucketPath =
	// BucketURL.CREATE_BUCKET.getUrl().formatted(BUCKET_NAME);
	// URI uri = URI.create("%s/%s".formatted(serverProperties.getBaseUrl(),
	// createBucketPath));
	// HttpRequest httpRequest = createHttpRequest(uri,
	// BucketExamples.EMPTY_BUCKET_SETTINGS_BODY);
	// doReturn(200).when(mockHttpResponse).statusCode();
	// doReturn(mockHttpResponse).when(httpClient).send(httpRequest,
	// HttpResponse.BodyHandlers.discarding());
	// String result = bucketClient.createBucket(BUCKET_NAME,
	// BucketSettings.builder().build());
	//
	// assertEquals(BUCKET_NAME, result);
	// }
	//
	// @Test
	// void createBucket_settingsProvided_bucketCreated() throws IOException,
	// InterruptedException {
	// String createBucketPath =
	// BucketURL.CREATE_BUCKET.getUrl().formatted(BUCKET_NAME);
	// URI uri = URI.create("%s/%s".formatted(serverProperties.getBaseUrl(),
	// createBucketPath));
	// HttpRequest httpRequest = createHttpRequest(uri,
	// BucketExamples.SAMPLE_BUCKET_SETTINGS_BODY);
	// doReturn(200).when(mockHttpResponse).statusCode();
	// doReturn(mockHttpResponse).when(httpClient).send(httpRequest,
	// HttpResponse.BodyHandlers.discarding());
	// String result = bucketClient.createBucket(BUCKET_NAME,
	...
	//
	// @Test
	// void createBucket_settingsNull_bucketCreated() throws IOException,
	// InterruptedException {
	// String createBucketPath =
	// BucketURL.CREATE_BUCKET.getUrl().formatted(BUCKET_NAME);
	// URI uri = URI.create("%s/%s".formatted(serverProperties.getBaseUrl(),
	...
	//
	// @Test
	// void createBucket_ioException_throwsException() throws IOException,
	// InterruptedException {
	// String createBucketPath =
	// BucketURL.CREATE_BUCKET.getUrl().formatted(BUCKET_NAME);
	// URI uri = URI.create("%s/%s".formatted(serverProperties.getBaseUrl(),
	...
	//
	// @Test
	// void createBucket_interruptedException_throwsException() throws IOException,
	// InterruptedException {
	// String createBucketPath =
	// BucketURL.CREATE_BUCKET.getUrl().formatted(BUCKET_NAME);
	// URI uri = URI.create("%s/%s".formatted(serverProperties.getBaseUrl(),
	...
	//
	// @Test
	// void createBucket_bucketAlreadyExists_throwsException() throws IOException,
	// InterruptedException {
	// String createBucketPath =
	// BucketURL.CREATE_BUCKET.getUrl().formatted(BUCKET_NAME);
	// URI uri = URI.create("%s/%s".formatted(serverProperties.getBaseUrl(),
	...
	// HttpRequest httpRequest = createHttpRequest(uri,
	// BucketExamples.SAMPLE_BUCKET_SETTINGS_BODY);
	// Optional<String> errorHeader = Optional.of("Bucket 'test-bucket' already
	// exists");
	...
	// () -> bucketClient.createBucket(BUCKET_NAME,
	// BucketSettings.builder().build()));
	//
